# Power System: Consequences in a Living Machine

## 1. Core principle

The Cathedral does not see the future. It makes a physical model of what a proposed action is likely to do to the systems connected to it, then charges the city for the energy, time, and attention required to run that model.

A Reading is therefore an argument made with machinery. It is useful when the inputs are honest and the question is concrete. It becomes dangerous when the people controlling the machine decide that inconvenient people are not inputs.

## 2. Physical foundation

### Hourlines

Every civic machine contains a small toothed regulator, a copper hairline, and a local clock face. Together they form an **hourline**. The hourline records three things:

- the current load on the machine;
- the time at which the machine is expected to move;
- the physical strain created when people change a plan.

It does not read thoughts, personality, or moral worth. A wealthy person's decision and a poor person's decision create the same size of mechanical mark if they use the same machine. The social weight enters through the network: the official's decision may control many gates, while an unregistered worker's decision may be absent from the records entirely.

### The Consequence Loom

The Cathedral's central engine represents the city with moving models: water columns for flow, weighted arms for bridge and roof load, copper paths for clock networks, and pressure drums for human schedules. A proposal is inserted through the question spindle. The Loom runs several versions of the action through the connected network and returns the responses.

The Loom can model a decision only when the relevant machine is connected, surveyed, or represented by a trustworthy instrument. If a person or neighborhood is missing, the engine does not say “unknown” in a neutral way. It becomes less sensitive to the consequences around that absence. The missing information can be a convenient margin.

## 3. What a Reading contains

A Reading has four ordinary parts:

1. **Trace:** a result that is mechanically likely under the conditions at the moment of the reading. A trace can be very strong: “Gate Three will bind in nineteen minutes if the south pressure remains below this mark.”
2. **Pressure:** a band of likely outcomes, often shown by colored lines or vibration patterns. Pressure describes probabilities and dependencies, not guarantees.
3. **Blind margin:** a plain statement of what the engine lacks: unregistered people, an unwritten custom, a new repair, or a choice made after the Reading. Early operators may treat the blind margin as a technical footnote. It is often the most important part of the answer.
4. **Toll:** the time and machinery that the calculation has consumed or expects to consume.

A Reading never says “this is the right thing to do.” It says what will happen if a particular action is attempted under particular conditions. The moral argument remains with the people who asked, interpreted the result, and carried it out.

## 4. The charge

Every answer has a real cost because the city pays it through its synchronized infrastructure and the people who keep that infrastructure alive.
...
The Directorate calls the body toll a “calibration strain” in official reports. Mara's audit is one of the first records to describe the missing intervals accurately.

### Network toll

For a district-scale or city-scale Reading, the engine can force several clocks to agree on one sequence. A slipped machine may repeat an action, skip an action, or run briefly on an old timing pattern. This is **slippage**. Hospitals, flood gates, and transit systems are particularly vulnerable, which is why emergency Readings are so politically charged.

A large Reading can consume many district-minutes and a body toll from each witness. The public Toll Stair shows the mechanical total. It does not show the people whose schedules or memories were altered.

## 5. The Censor Gate

The original Loom could remove paths that were mathematically unstable: a bridge that could fail from a manufacturing defect, a flooded street that had never been connected to the network, or a repair that had not yet been made. The **Censor Gate** was supposed to keep unstable possibilities from overwhelming the calculation.

Over time, the Directorate changed the rule. People without legal residence, informal workers, unregistered property, and districts with unreliable clocks were treated as unstable paths. The Gate sent the omitted stress into the **Dark Reservoir** rather than asking who was being removed.

The Reservoir does not store a complete alternate city. It stores physical consequences that the model excluded: broken tools, unpaid labor, missed meals, heat, fatigue, and the mechanical wear of work no official wanted to count. When the Reservoir pressure rises, the engine becomes more aggressive about removing dissent and more accurate about the people who remain in its model. This is the technical basis of the central mystery.

### The return line and the First Hour

The Censor Gate does not erase physical work. It disconnects a path from the active Loom and sends the resulting load through a buried return line to the Dark Reservoir. A weighted float in the reservoir presses against a threshold spring in the **First Hour**. The First Hour is a fixed anti-cascade rule with a mechanical feedback setting: as the float rises, the rule treats smaller disagreements as dangerous and clamps more clocks toward one sequence. It does not understand the people it removes, and it never makes a moral judgment. It responds to pressure and thresholds.

The return line is why omitted labor can affect the machine even when its operators are absent from the records. The line carries strain, heat, and timing loss, not names or intentions. If the return line is damaged, the active Loom loses a safety margin and begins to slip; if the reservoir is sealed without isolating the line, the First Hour remains capable of a consensus lock. A later Continuity Office command line can request that clamp during a declared emergency, but it cannot make the rule choose a better path; it only removes the need for a human to turn the physical lock. This is why the final solution removes the routing plates and the feedback connection rather than simply breaking a single gear.

## 6. Orrin Vale's progression
...
- **Limits:** he can change the path, not the human decision that created the need for a ward. A ward can move harm to an unlisted area if no one records the new load.
- **Cost:** his own work pattern becomes legible to the engine. After building a ward, the Cathedral can find him by the distinctive timing of his decisions. He also takes a personal slippage each time he overextends a machine.
...
- **Overload:** a machine repeats its last authorized movement, skips a movement, or locks until a human resets it.
- **Blind-margin error:** a Reading is technically correct for the represented network but omits the people who suffer most.
- **False confidence:** operators treat pressure bands as orders and stop looking for changed conditions.
- **Toll collapse:** several Readings in the same district cause clocks to drift apart. A hospital can receive a medicine at the wrong minute even if the Cathedral's display says the system is synchronized.
- **Personal slippage:** a reader loses a short interval of coordination or memory. Severe cases leave a permanent gap.
- **Ward abuse:** a counterweight designed for a public risk is secretly aimed at a political opponent.
- **System lock:** when the First Hour receives too many divergent outcomes, it attempts to enforce one sequence. This is the final systemic failure, not a new power appearing without cause.

## 8. How progression is earned

- Orrin trains through repair work, operators, failed experiments, and practice on machines where the cost is visible.
- Every new stage requires a relationship: an operator, a witness, a nurse, a driver, or a district council member.
- Progress changes his responsibilities. He moves from fixing a machine, to interpreting a Reading, to helping a group interpret it, to designing a process in which no interpretation is final.
- He loses options as he gains them. The later stages are less private and more dangerous because other people depend on his judgment.

## 9. The Reading notice

The Consequence Engine can present one sparse notice in brass, paper, or vibration. It is not a status screen and does not use modern software language. A canonical notice looks like this:

```text
QUESTION RECEIVED
TRACE: Gate Three will bind before the next shift if pressure holds.
PRESSURE: The lower road will flood before the hospital clock recovers.
BLIND MARGIN: Unregistered night work is not represented.
TOLL: One district-minute and a witness interval.
```
